...
const Cart = () => {
    ...
    useEffect(() => {
        let totalPriceArray = thingsOfCart.map(el => el.price * el.quantity)
    ...
    const cartThingsDecrease = async (id) => {
        try {
            const response = await fetch(`http://127.0.0.1:8000/decrease-thing-cart/${id}/`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + String(tokens.access)
    ...
    return (
        <>
...

            {
                ...
                <Loading />
                    :
                <div style={thingsOfCart.length <= 0 ? {padding: "0", margin: "0"} : null} className="cart">
                    {
                                    ...
                                    thingsOfCart.map((thing, id) => (
                                        <div className="container__cart_thing" key={id}>
                                            ...
                                            <div className="cart_thing__quantity">
                                                <button onClick={() => cartThingsIncrease(thing.id)}>
                                                ...
                                                <button onClick={() => cartThingsDecrease(thing.id)}>
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
                                            ...
                                            <div className="cart_thing__price_delete">
                                                ...
                                                <button onClick={() => cartThingsDelete(thing.id)}>
                                                    <svg className="delete_cart" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 256 256" enableBackground="new 0 0 256 256" xmlSpace="preserve">
